        ...
        <Sheet>
          ...
          <SheetContent className='sheet-content justify-around flex items-start flex-col min-h-screen   border-2 bg-opacity-80 border-s2 bg-black shadow-md shadow-purple-200/50  sm:w-64 '>
            <>
              ...
              <ul className="w-full flex-col items-start ">
                {navLinks.slice(0, 9).map((link) => {
                  const isActive = link.URL === location;
                  return (
                    <li
                      key={link.id}
            ...
            {navLinks.slice(9).map((link) => (
              <li
                key={link.id}
                className="w-full whitespace-nowrap overflow-hidden rounded-full hover:bg-opacity-70 transition-all duration-150 ease-in-out flex items-center justify-start text-[16px] font-semibold leading-[140%]"
              >
                <button
                  onClick={() => window.open(link.URL, "_blank")}
                  className="relative w-full gap-2 shadow-200 shadow-gray-800 rounded-full flex flex-row justify-start items-center group"
                >
                  <span className="relative min-w-fit flex justify-around p-1 items-center w-auto before:g7 g4 min-h-fit rounded-full before:absolute before:inset-0 before:opacity-0 before:transition-opacity before:duration-500 before:content-[''] group-hover:before:opacity-100 overflow-hidden">
                    {link.icon && (
                      <div className={`flex justify-center rounded-full items-center  ${link.name === "LinkedIn" ? "bg-blue-600" : "bg-white"} h-7 w-7 p-1.5`}>
                        {link.name === "LinkedIn"?
                            <Linkedin className=" fill-white object-contain "/>
                            :
                            <Github className={`object-contain fill-black text-black`}/>
                          }
                      </div>
                    )}
                  </span>
                  <span className="z-2 font-semibold tracking-wide normal-case rounded-full font-poppins text-[9px] mr-2 text-white">
                    {link.name}
                  </span>
                </button>
              </li>
